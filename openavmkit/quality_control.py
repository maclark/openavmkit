...
def check_land_values(
  ...
  df["model_land_alloc"] = div_field_z_safe(df["model_land_value"], df["model_market_value"])
  df["model_impr_value"] = df["model_market_value"] - df["model_land_value"]
  df["model_impr_alloc"] = div_field_z_safe(df["model_impr_value"], df["model_market_value"])

  derived_land_alloc = div_field_z_safe(df["model_land_value"], df["model_market_value"])
  ...
  print("")
  if any(counts.values()):
    warnings.warn(f"Remaining issues after error correction for model group {model_group}:")
    for key, value in counts.items():
      if value:
        label = labels[key]
        perc = value / n
...
def _perform_land_checks(df: pd.DataFrame, counts: dict, do_remedy:bool) -> dict:
  ...
  if do_remedy:
    ...
    df.loc[idx_negative_land, "model_impr_value"] = df.loc[idx_negative_land, "model_market_value"]
  ...
  if do_remedy:
    ...
    df.loc[idx_market_lt_land, "model_land_value"] = df.loc[idx_market_lt_land, "model_market_value"]
  ...
  idx_land_gt_market_vacant = df["model_land_value"].gt(df["model_market_value"]) & df["bldg_area_finished_sqft"].eq(0)
  idx_land_gt_market_improved = df["model_land_value"].gt(df["model_market_value"]) & df["bldg_area_finished_sqft"].ge(1)
  ...
  if do_remedy:
    ...
    df.loc[idx_land_gt_market, "model_land_value"] = df.loc[idx_land_gt_market, "model_market_value"]
  ...
  idx_bldg_yes_land_alloc_ge_1 = df["bldg_area_finished_sqft"].ge(1) & df["model_land_alloc"].ge(1)
  ...
  idx_bldg_no_land_alloc_ne_1 = df["bldg_area_finished_sqft"].eq(0) & df["model_land_alloc"].ne(1)
  counts["bldg_no_land_alloc_ne_1"] = idx_bldg_no_land_alloc_ne_1.sum()
  if do_remedy:
    # Remedy 4: set land value to market value
    df.loc[idx_bldg_no_land_alloc_ne_1, "model_land_value"] = df.loc[idx_bldg_no_land_alloc_ne_1, "model_market_value"]
