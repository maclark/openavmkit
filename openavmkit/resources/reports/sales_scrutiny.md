...
Traditionally, sales are validated from transaction details such as deed type, with the aim of identifying non-arms-length transfers. However, sales can also be statistically validated based on the following principle: 

> Similar properties sold in similar locations should have similar prices, especially on a price/sqft basis.

Similar properties sold in similar locations that do *not* have similar prices, would therefore indicate a potential anomaly -- either in the sale itself, or in the property characteristics.

(All prices in this analysis are time-adjusted, taking temporal variation out of the equation.)

We divide the sales into clusters of similar properties, then look for sales that stick out within their groups. If we find a statistical anomaly, that can be a sign of an invalid sale, incorrect property characteristics, or missing information. Not every anomaly indicates an invalid sale, but they should all be manually reviewed by {{{locality}}}. 

We flagged **{{{num_sales_flagged}}}** anomalous sales out of **{{{num_sales_total}}}** sales total for the **{{{modeling_group}}}** modeling group.

Here is a breakdown of anomalies by type:

| Type of Flagged Sale                                                   | Number of Sales                | % of Total Sales               |
|------------------------------------------------------------------------|--------------------------------|--------------------------------|
| 1 - Price in range, high OR low price/sqft, high OR low square footage | {{{num_sales_flagged_type_1}}} | {{{pct_sales_flagged_type_1}}} |
...
| **Total flagged sales**                                                | **{{{num_sales_flagged}}}**    | **{{{pct_sales_flagged}}}**    |
| **Total overall sales**                                                | **{{{num_sales_total}}}**      | **100%**                       |
...
|---|----------|------------|----------------|--------------------------------------------------------------------------------|---------------------------------------------------------|
| 1 | In range | High/Low   | High/Low       | Incorrect square footage                                                       | Check square footage / characteristics at time of sale. | 
...
## 1. In-range price, anomalous price/sqft, anomalous square footage
