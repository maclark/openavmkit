...
def derive_prices(
  ...
  The model assumes:
    ...
    - At exit, the property is sold at:
      ...
      and that sale price is discounted back to time 0.

  The DCF equation is:

    Asking Price = Σₜ₌₁ᴴ [ NOI₀*(1+noi_growth)^t / (1+target_irr)^t ]
                   + [ NOI₀*(1+noi_growth)^(H+1) / exit_cap_rate ] / (1+target_irr)^H
...
"""
  ...
  #
  #
  # # Calculate the present value of the annual NOI cash flows
  # pv_noi = sum([
  #   entry_noi * (noi_mult ** t) / (irr_mult ** t) for t in range(1, holding_period + 1)
  # ])
  #
  # # Calculate the terminal (exit) value based on the exit cap rate, discounted back to time 0.
  # terminal_value = entry_noi * (noi_mult ** (holding_period + 1))
  # terminal_discount = (exit_cap_rate * (irr_mult ** holding_period))
  # pv_terminal = terminal_value / terminal_discount
  #
  # asking_price_dcf = pv_noi + pv_terminal
  #
  # # Optionally, you might compare this DCF asking price with the value implied by the entry cap rate:
  # value_entry = entry_noi / entry_cap_rate
  # # For instance, a conservative asking price might be the lower of the two.
  # # Here we simply return the DCF value.
  #
  # return asking_price_dcf
...
def derive_irr(
  ...
  The function returns the IRR (as a decimal).
  """
  ...
  def dcf_equation(IRR: float) -> float:
    ...
    pv_noi = sum([entry_noi * (1 + noi_growth) ** t / (1 + IRR) ** t for t in range(1, holding_period + 1)])
...
def derive_noi_growth(
  ...
  implied_growth = brentq(_npv_with_growth, a=lower_bound, b=upper_bound, args=(
    entry_price,
    entry_noi,
    target_irr,
    holding_period,
...
def calculate_cap_rate_growth(sale_price_growth: np.ndarray, noi_growth: np.ndarray) -> np.ndarray:
  """
  Calculate the capitalization rate given the annual percentage changes in sale price ($/sqft)
  and net operating income (NOI).

  Parameters:
      sale_price_growth (list or array-like):
          Annual percentage changes in sale price as decimals.
          Example: [0.01, 0.01, 0.015, 0.02, 0.01] for 1%, 1%, 1.5%, 2%, 1%.
      noi_growth (list or array-like):
          Annual percentage changes in NOI as decimals.
          Example: [0.01, 0.005, 0.0, -0.0025, 0.01].

  Returns:
      np.ndarray: Capitalization rate as decimals.
...
def calculate_noi_growth(sale_price_growth: np.ndarray, cap_rate_growth: np.ndarray) -> np.ndarray:
  """
  Calculate the annual percentage change in NOI given the annual percentage changes in
  sale price ($/sqft) and cap rate.

  Parameters:
      sale_price_growth (list or array-like):
          Annual percentage changes in sale price as decimals.
          Example: [0.01, 0.01, 0.015, 0.02, 0.01] for 1%, 1%, 1.5%, 2%, 1%.
      cap_rate_growth (list or array-like):
          Annual percentage changes in cap rate as decimals.
          Example: [0.01, 0.005, 0.0, -0.0025, 0.01].

  Returns:
      np.ndarray: Annual percentage changes in NOI as decimals.
...
def derive_irr_df(
  ...
  Returns a new DataFrame that includes the original columns plus these computed fields.
  """
  def compute_row(row):
